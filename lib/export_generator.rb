  ...
  def to_photowall_pdf
    @child_data.each do |child|
      ...
      @pdf.start_new_page unless @child_data.last == child
		...
		return Export.new @pdf.render, {:type=>"application/pdf", :filename=>filename("photowall", "pdf")}
  end
	def to_csv
    ...
    @child_data.each do |child|
      ...
      @pdf.start_new_page unless @child_data.last == child
		...
		return Export.new @pdf.render, {:type=>"application/pdf", :filename=>filename("fulldetails", "pdf")}
  end
